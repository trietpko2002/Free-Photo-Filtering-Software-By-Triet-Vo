...
* **[v7.0] - Ra tính năng mới : Đánh giá hình ảnh bằng AI:** Sử dụng mô hình của Groq AI (Llama 3.2 Vision) để nhận xét, đánh giá hình ảnh và đưa ra gợi ý hậu kỳ theo cảm nhận của AI.
...
## 🤖 Hướng Dẫn Cài Đặt AI Nhận Xét Ảnh (Groq API Key)

Để sử dụng tính năng AI (Llama 3.2 Vision) nhận xét ảnh cực nhanh, bạn cần có một API Key từ hệ thống Groq (hoàn toàn miễn phí):

1. Truy cập trang web: [https://console.groq.com/keys](https://console.groq.com/keys)
2. Đăng nhập hoặc Tạo một tài khoản mới.
3. Nhấn vào nút **Create API Key**, đặt tên cho key và copy chuỗi mã (thường bắt đầu bằng `gsk_...`).
4. Mở phần mềm Photo Filter Studio, vào mục **⚙️ CÀI ĐẶT**.
5. Dán mã vừa copy vào ô **API Key Groq** và nhấn nút **💾 LƯU CẤU HÌNH**.
...
* **Kiến trúc hệ thống:** * `QFileSystemWatcher` (Giám sát file thời gian thực).
  * `QThread` (Download & Xử lý API AI đa luồng).
  * Tương tác API REST (GitHub API, Groq AI API).
...
**Tác giả:** Triet Vo  
**Phiên bản hiện tại:** v7.0 STABLE STUDIO  
**Mã nguồn / Góp ý:** Hãy tạo một [Issue](https://github.com/trietpko2002/web_gioi_thieu_phan_mem_loc_anh_v8/issues) nếu bạn phát hiện lỗi hoặc muốn đóng góp tính năng mới!
